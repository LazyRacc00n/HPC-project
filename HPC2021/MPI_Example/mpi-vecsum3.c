...
int main( int argc, char* argv[] )
{
    ...
    for (i=0; i<comm_sz; i++) {
        ...
        //index of starting point of data of proc i
        const int start = n*i/comm_sz;
        //index of starting point of data of proc i+1
        const int end = n*(i+1)/comm_sz;
        //number of elements that will be contained in proc i
        counts[i] = end - start;
        printf("\nCounts[%d] = %d\n", i, counts[i]);
        //where start the data in the send_buffer that will be sent to proc i
        displs[i] = start;
        printf("\ndispls[%d] = %d\n", i, displs[i]);
    }
